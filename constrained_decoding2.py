...
def bracket_constraint_decode(
    ...
    if model_kwargs["past_key_values"] is not None:
        tmp = move_past_to_cpu(model_kwargs["past_key_values"])
        model_kwargs["past_key_values"] = None
        model_kwargs["past_key_values"] = tmp
...
def move_past_to_cpu(past):
    cpu_past = ()
    for layer_past in past:
        cpu_past += (tuple(past_state.cpu() for past_state in layer_past),)
    return cpu_past


def reorder_cache(past, beam_idx, device=None):
    reordered_past = ()
    if device is not None:
        for layer_past in past:
            reordered_past += (tuple(past_state.to(device).index_select(0, beam_idx) for past_state in layer_past),)
    else:
        for layer_past in past:
